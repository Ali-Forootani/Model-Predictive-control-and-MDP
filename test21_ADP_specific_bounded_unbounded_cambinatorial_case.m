...
%mu=[0.2;0.2;0.4];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
lambda_max=[0.6;0.5;0.3];
mu_max=[0.2;0.2;0.4];
lambda_min=[0.6;0.5;0.3];
mu_min=[0.2;0.2;0.4];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=T:-1:1
    sample_length=sample_length+i-T;
    D_h(1:m,i)=D(1:m,i);
    F_h(1:m,i)=F(1:m,i);
    [sample_space_1] = state_space_generation_p(m,N,sample_length,D_h+F_h,1);
    [r_k_horizon_h_1]= r_k_horizon_generation_unbounded_bounded_p(m,N,sample_space_1,sample_length,lambda_max ...
    ,lambda_min,mu_max,mu_min,C,D_h,F_h,2,r_hat);
    r_k_horizon_h(1:m+1,T)=r_hat;
    r_hat=r_k_horizon_h_1;
end


%D_h=D;
%F_h=F;
%[sample_space] = state_space_generation_p(m,N,sample_length,D_h+F_h,T);
...
if f_3 <= 0.8
    %D_h=D;
    %F_h=F;
    h_1=randi([n45+5 n45+10],1,1);
    h_2=randi([h_1 h_1+10],1,1);
    l=h_2;
    ox_D=1;
    ox_F=0;
else
...
if n45>=2
r_kk=r_k_horizon_h(:,n45-1);
r_kk_p=r_k_horizon_h(:,n45);
D_kk=D(:,n45-1);
D_kk_p=D(:,n45);        
F_kk=F(:,n45-1);
F_kk_p=F(:,n45);

[Decision] = Exact_ADP_Booking_unbounded_bounded_12(initial_state,N,m...
        ,D_kk,D_kk_p,F_kk,F_kk_p,lambda_max,lambda_min,mu_max,mu_min,C,r_kk,r_kk_p);
    
[initial_state_2] = next_state_generation_ADP_Booking_unbounded_bounded_12(initial_state,N,m,D_kk,...
D_kk_p,F_kk,F_kk_p,lambda_max,lambda_min,mu_max,mu_min,C,Decision);   
...
if ox==1 & ox_D==1 & ox_F==0

%if ox_D==1 & ox_F==0
    n55=n55+1;
    bounded_reservations(n55,1:2)=[h_1 h_2];
    bounded_reservations(n55,3)=n45;
%end

end


if ox==0 & ox_D==1 & ox_F==0
    
    n66=n66+1;
    rej_bounded_reservations(n66,1:2)=[h_1 h_2];
    bounded_reservations(n55,3)=n45;
    
end


if ox==1 & ox_F==1 & ox_D==0

%if ox_F==1 & ox_D==0
    n56=n56+1;
    unbounded_reservations(n56,1:2)=[h_1 h_2];
    unbounded_reservations(n56,3)=n45;
%end

end

if ox==0 & ox_F==1 & ox_D==0

%if ox_F==1 & ox_D==0
    n77=n77+1;
    unbounded_reservations(n77,1:2)=[h_1 h_2];
    unbounded_reservations(n77,3)=n45;
%end
...
ox
l=h_2;
initial_state_visitis(n45,1:m) = initial_state;
...
while n <= num_exper
    ...
    while n1<=l-2
        ...
        state;
        Expected_Revenue=Discount_Factor*Expected_Revenue+Addition(state,C);
...
if Decision_reservation ~= m+1
    ...
    if ox_D==1 & ox_F==0
        n61=n61+1;
        c_bounded_reservation(n61,1:2)=[h_1 h_2];
    end
    
    if ox_D==0 & ox_F==1
        n62=n62+1;
        c_unbounded_reservation(n62,1:2)=[h_1 h_2];
    end
...
[sample_space] = state_space_generation(m,N,sample_length,D_b+F_b,T);

%%

[r_k_horizon_b]= r_k_horizon_generation_unbounded_bounded(m,N,sample_space,sample_length,lambda_max ...
,lambda_min,mu_max,mu_min,C,D_b,F_b,T);
