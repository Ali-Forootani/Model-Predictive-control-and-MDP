function [Revenue] = DP_Space_sampling_unbounded_bounded_p(m,N,sample_space,sample_space_p...
    ,D,F,lambda_max,lambda_min,mu_max,mu_min,T,k_p,C,r_hat);
...
% for d=1:m
%     p(d)=g(d)+1;
%     De(1:m,1:10,d)=D(1:m,21:30);
%     De(1:m,1:3,d)=De(1:m,1:3,d)+p;
%     p(d)=0;
% end
...
while n1<= NS(1,1)
    %[ok] = state_difference(S_p(n1,1:end),D(1:m,k_p));
    [ok] = state_difference_unbounded(S(n1,1:end),D(1:m,end),F(1:m,end));
    if ok==1
        y3=phi_cal2(m,S_p(n1,1:end));
        y1=y3'*r_hat;
        %y1=Addition(S(n1,1:end),C);
        Terminal_revenue(n1,1:(m+1),1,d)=[S_p(n1,1:end), y1];
    else
        Terminal_revenue(n1,1:(m+1),1,d)=[S_p(n1,1:end), 0];
    end
    n1=n1+1;
end
...
n2=1;
k=1;
Revenue(1:NS(1,1),1:(m+1),2,d)=Terminal_revenue(1:NS(1,1),1:(m+1),d);
Revenue(1:NS(1,1),(m+2),2,d)=0;
...
while k>= 1
    ...
    if norm(D_k_p+F_k_p,1)==N & norm(D_k+F_k,1)<N
        while n1<= NS(1,1)
            ...
            [ok] = state_difference_unbounded(S(n1,1:end),D_k,F_k);
            if ok == 1
                if sum(S(n1,1:end))<=N
                    transition=D_k_p'+F_k_p';
                    combined_probability=ones(1,m);
                    %x=Revenue(1:end,1:end,k+1,d);
                    %Rev=Pair_finding_2(transition,x,NS(1,1),combined_probability);
                    Rev=value_to_go(transition,combined_probability,r_hat);
                    Revenue(n1,(m+1),k,d)=Addition(S(n1,1:end),C)+Discount_Factor*Rev;
                    Revenue(n1,1:m,k,d)=S(n1,1:end);
                    Revenue(n1,(m+2),k,d)=0;
                end
            else
                    Revenue(n1,1:m,k,d)=S(n1,1:end);
                    Revenue(n1,(m+1),k,d)=0;
                    Revenue(n1,(m+2),k,d)=0;
            end
            n1=n1+1;
        end
        ...
        while n1<= NS(1,1)
                ...
                while a <= m
                    if C(a)~=0
                       ...
                       if ok==1
                        ...
                        [state_status]=eligible_state_unbounded(transition,D_k_p,F_k_p,m,N);
                        %x=Revenue(1:end,1:end,k+1,d);
                        %Revenue(1:end,1:end,k+1,d);
            ...
            if indicator_rej==1
                ...
                if ok==1
                    ...
                    [state_status]=eligible_state_unbounded(transition_re,D_k_p,F_k_p,m,N);
                    x_re=Revenue(1:end,1:end,k+1,d);
                    if state_status==1
                           transition_3 = shifted_transition_unbounded(transition_re,D_k_p,F_k_p,m);
                           %Rev=Pair_finding(transition_3,x,NS(1,1),combined_probability);
                           Rev=value_to_go(transition_3,combined_probability_re,r_hat);
            ...
            if ok==1
                ...
                Revenue(n1,(m+1),k,d)=Addition(S(n1,1:end),C)+Discount_Factor*max_revenue_state;
                Revenue(n1,1:m,k,d)=S(n1,1:end);
                Revenue(n1,(m+2),k,d)=price;
            else
                Revenue(n1,1:m,k,d)=S(n1,1:end);
                Revenue(n1,(m+2),k,d)=0;
            end
    ...
    while n1<= NS(1,1) 
        ...
        if sum(S(n1,1:end))<N
                ...
                if C(a)~=0
                    ...
                    if ok==1
                       ...
                       if state_status==1
                           ...
                           transition_2 = shifted_transition_unbounded(transition,D_k_p,F_k_p,m);
                           %Rev=Pair_finding(transition_2,x,NS(1,1),combined_probability);
                           Rev=value_to_go(transition_2,combined_probability,r_hat);
            ...
            if ok==1 & state_feasibility_3==0
                ...
                while ok_2==0
                    ...
                    if state_status_5==1
                        ok_2=1;
                        transition_6 = shifted_transition_unbounded(SS_p,D_k_p,F_k_p,m);
                        combined_probability_6=ones(1,m);
                        %Rev=Pair_finding(transition_6,x,NS(1,1),combined_probability);
                        Rev=value_to_go(transition_6,combined_probability_6,r_hat);
                        Revenue(n1,(m+1),k,d)=Addition(S(n1,1:end),C)+Discount_Factor*Rev;
                        Revenue(n1,1:m,k,d)=S(n1,1:end);
                        Revenue(n1,(m+2),k,d)=m+2;
                    else
        ...
        if sum(S(n1,1:end))<N
            while a <= (m) 
                if C(a)~=0
                    ...
                    if ok==1
                       ...
                       [state_status]=eligible_state_unbounded(transition_zero,D_k_p,F_k_p,m,N);
                       ...
                       if state_status==1
                           ...
                           transition_2_zero = shifted_transition_unbounded(transition_zero,D_k_p,F_k_p,m);
                           %Rev=Pair_finding(transition_2_zero,x,NS(1,1),combined_probability);
                           Rev=value_to_go(transition_2_zero,combined_probability,r_hat);
                           possible_Revenue(1,a)=Rev;
                       else
                           possible_Revenue(1,a)=0;
                       end
            ...
            if indicator_rej==1
                ...
                if ok==1
                    ...
                    [state_status]=eligible_state_unbounded(transition_re_zero,D_k_p,F_k_p,m,N);
                    %x_re=Revenue(1:end,1:end,k+1,d);
                    if state_status==1
                           state_feasibility=state_feasibility+1;
                           S_p;
                           k;
                           transition_3_zero = shifted_transition_unbounded(transition_re_zero,D_k_p,F_k_p,m);
                           %Rev=Pair_finding(transition_3_zero,x,NS(1,1),combined_probability);
                           Rev=value_to_go(transition_3_zero,combined_probability_re,r_hat);
                           possible_Revenue(1,1+m)=Rev;
                       else
                           possible_Revenue(1,1+m)=0;
                    end
        ...
        if sum(S(n1,1:end))==N
            ...
            if ok==1
                ...
                [state_status]=eligible_state_unbounded(transition_ca,D_k_p,F_k_p,m,N);
                %x_ca=Revenue(1:end,1:end,k+1,d);
                transition_ca;
                if state_status==1
                           transition_4 = shifted_transition_unbounded(transition_ca,D_k_p,F_k_p,m);
                           state_feasibility_2=state_feasibility_2+1;
                           %Rev_ca=Pair_finding(transition_4,x_ca,NS(1,1),combined_probability_ca);
                           Rev_ca=value_to_go(transition_4,combined_probability_ca,r_hat);
                           possible_Revenue_ca=Rev_ca;
                           Revenue(n1,(m+1),k,d)=Addition(S(n1,1:end),C)+ Discount_Factor*possible_Revenue_ca;
                           Revenue(n1,1:m,k,d)=S(n1,1:end);
                           Revenue(n1,(m+2),k,d)=m+1;
                %else
                %        possible_Revenue(1,1+m)=0;
                %         Revenue(n1,1:m,k)=S(n1,1:end);
                %         Revenue(n1,(m+2),k)=0;
            ...
            if ok==1 & state_feasibility_2==0
                ...
                while ok_2==0
                    ...
                    if state_status_5==1
                        ...
                        %Rev=Pair_finding(transition_6,x,NS(1,1),combined_probability);
                        Rev=value_to_go(transition_6,combined_probability_6,r_hat);
                        Revenue(n1,(m+1),k,d)=Addition(S(n1,1:end),C)+Rev;
                        Revenue(n1,1:m,k,d)=S(n1,1:end);
                        Revenue(n1,(m+2),k,d)=m+2;
                    else
                        er=er+1;
